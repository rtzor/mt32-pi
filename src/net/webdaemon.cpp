...
THTTPStatus CWebDaemon::BuildSoundPage(u8* pBuffer, unsigned* pLength, const char** ppContentType)
{
		...
		html.Append("const tabMT32=document.getElementById('tab-mt32');if(tabMT32)tabMT32.addEventListener('click',()=>rtApply('active_synth','mt32'));const tabSF=document.getElementById('tab-sf');if(tabSF)tabSF.addEventListener('click',()=>rtApply('active_synth','soundfont'));const tabOPL3=document.getElementById('tab-opl3');if(tabOPL3)tabOPL3.addEventListener('click',()=>rtApply('active_synth','opl3'));");
